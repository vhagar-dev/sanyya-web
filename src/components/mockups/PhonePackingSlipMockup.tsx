...
export function PhonePackingSlipMockup({ className }: { className?: string }) {
  return (
    <div className={cn("relative mx-auto w-full max-w-[280px] sm:max-w-xs md:max-w-sm", className)}>
      {/* Phone frame */}
      <div className="relative aspect-[9/17] rounded-[2.5rem] border border-border bg-[hsl(220_15%_18%)] p-2 shadow-[0_1px_2px_hsl(220_43%_11%/0.06)]">
        ...
        <div className="relative h-full w-full overflow-hidden rounded-[2rem] bg-card">
          {/* Status bar */}
          <div className="flex items-center justify-between px-5 pt-3 tabular-nums text-[10px] text-muted-foreground">
            <span>9:41</span>
            <span>•••</span>
          </div>
          ...
          <div className="relative mx-3 overflow-hidden rounded-xl border border-border bg-[hsl(0_0%_98%)] p-3">
            <div className="space-y-1 text-[8px] text-[hsl(0_0%_30%)]">
              ...
              <div className="h-1" />
              <div className="rounded ring-2 ring-[hsl(268_70%_52%)]/80 px-1 py-0.5 bg-[hsl(268_70%_52%/0.15)]">
                Lot # FBS-2024-A881
              </div>
              <div>Item: Fetal Bovine Serum, 500mL</div>
              <div>Qty: 12 bottles</div>
              <div className="rounded ring-2 ring-[hsl(168_78%_60%)]/80 px-1 py-0.5 bg-[hsl(168_78%_60%/0.15)]">
                Ship Temp: -20°C
              </div>
          ...
          {/* Matched line items */}
          <div className="mx-3 mt-3 overflow-hidden rounded-xl border border-border">
            <div className="flex items-center justify-between bg-secondary/70 px-2.5 py-1.5">
              <span className="tabular-nums text-[9px] text-muted-foreground">
                ordered vs received
              </span>
              <span className="tabular-nums text-[9px] text-muted-foreground">PO-4821</span>
            </div>
            <LineRow name="Fetal Bovine Serum, 500 mL" ordered={12} received={12} />
            <LineRow name="Cryovials, 2 mL, case" ordered={6} received={4} />
            <LineRow name="Nitrile gloves, box" ordered={10} received={10} />
          </div>

          <div className="mx-3 mt-2 flex items-center justify-between rounded-lg border border-[hsl(25_90%_60%/0.35)] bg-[hsl(25_90%_60%/0.08)] px-2.5 py-1.5">
            <span className="text-[10px] text-foreground">
              2 items still outstanding on this order
            </span>
            <span className="tabular-nums text-[10px] text-[hsl(25_90%_40%)]">open</span>
          </div>
        </div>
      </div>
    </div>
...
function Corner({ className }: { className?: string }) {
  return <div className={cn("absolute h-3 w-3 border-[hsl(269_80%_57%)]", className)} />;
}
...
function Field({
  ...
  return (
    <div className="flex items-center justify-between rounded-lg border border-border bg-background/50 px-2.5 py-1.5">
      ...
      <span className="tabular-nums text-[10px] text-foreground">{value}</span>
    </div>
  );
}

function LineRow({ name, ordered, received }: { name: string; ordered: number; received: number }) {
  ...
  return (
    <div className="flex items-center justify-between gap-2 border-t border-border px-2.5 py-1.5">
      ...
      <div className="flex shrink-0 items-center gap-1.5">
        ...
        {short ? (
          <span className="rounded-full bg-[hsl(25_90%_60%/0.15)] px-1.5 py-0.5 tabular-nums text-[9px] text-[hsl(25_90%_40%)]">
            {ordered - received} out
          </span>
        ) : (
          <span className="rounded-full bg-[hsl(160_84%_50%/0.12)] px-1.5 py-0.5 tabular-nums text-[9px] text-[hsl(160_84%_35%)]">
            ok
          </span>
        )}
      </div>
