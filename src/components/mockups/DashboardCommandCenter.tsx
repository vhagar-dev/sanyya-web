    ...
    <div className={cn("relative w-full", className)}>
      <div className="glass relative overflow-hidden rounded-2xl p-3 sm:p-5 md:p-6 shadow-[0_1px_2px_hsl(220_43%_11%/0.06)]">
        <div className="mb-4 flex items-center justify-between">
          <div />
          <div className="tabular-nums text-[10px] text-muted-foreground">Dashboard</div>
        </div>

        {/* 2x2 quadrant grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-px rounded-xl bg-card/10 overflow-hidden">
          <ActivityPanel />
          <StatsPanel />
          <SpendPanel />
          <AIInsightPanel />
...
function QuadrantLabel({ children }: { children: React.ReactNode }) {
  return <div className="tabular-nums text-[10px] text-muted-foreground mb-3">{children}</div>;
}
...
function ActivityPanel() {
  const items = [
    ...
    {
      color: "bg-[hsl(269_80%_57%)]",
      text: "PO #4856 dispatched to Fisher Scientific",
      time: "18m",
    },
    { color: "bg-[hsl(272_75%_72%)]", text: "Requisition approved by Sarah M.", time: "1h" },
    { color: "bg-[hsl(38_92%_60%)]", text: "Low stock alert: Pipette Tips 200μL", time: "2h" },
    { color: "bg-[hsl(158_80%_60%)]", text: "GRN created from delivery scan", time: "3h" },
  ];
  return (
    ...
    <div className="bg-background/60 p-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        {stats.map((s) => (
          ...
          >
            <div className="tabular-nums text-[9px] text-muted-foreground">{s.label}</div>
      ...
      <div className="flex items-center gap-4">
        <div className="relative shrink-0">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <div className="tabular-nums text-[8px] text-muted-foreground">This Month</div>
          </div>
        </div>
    ...
    <div className="relative bg-background/60 p-4">
      ...
      <div className="relative">
        <div className="mb-3 flex items-center gap-1.5">
          <Sparkles className="size-3 text-[hsl(272_72%_62%)]" />
          <span className="tabular-nums text-[10px] bg-gradient-to-r from-[#3C83F5] via-[#895AF6] to-[#67E7F8] bg-clip-text text-transparent font-semibold">
            Sanyya AI
          </span>
        </div>
