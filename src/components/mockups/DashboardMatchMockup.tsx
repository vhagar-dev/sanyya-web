...
export function DashboardMatchMockup({ className }: { className?: string }) {
  return (
    <div className={cn("relative w-full", className)}>
      <div className="glass relative overflow-hidden rounded-2xl p-4 sm:p-6 md:p-8 shadow-[0_30px_80px_-30px_hsl(217_91%_57%/0.4)]">
        ...
        <div className="grid gap-3 md:grid-cols-3">
          <DocCard
            ...
            color="text-[hsl(214_90%_38%)]"
            ring="ring-[hsl(214_90%_60%/0.4)]"
            label="PO #4821"
        ...
        <div className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-[hsl(160_84%_50%/0.3)] bg-[hsl(160_84%_50%/0.07)] p-3">
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-xs text-[hsl(160_84%_40%)]">99.2%</span>
          </div>
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            <Sparkles className="size-3 text-[hsl(258_90%_38%)]" />
            <span className="font-mono">vector · semantic</span>
          </div>
        </div>
...
function DocCard({
  ...
  return (
    <div className={cn("rounded-xl border border-border bg-background/50 p-3 ring-1", ring)}>
      <div className={cn("mb-2 flex items-center gap-2 font-mono text-[10px] uppercase tracking-widest", color)}>
        {icon}
        {label}
      </div>
      <div className="text-sm font-medium text-foreground">{title}</div>
      <div className="mt-1 font-mono text-[11px] text-muted-foreground">{meta}</div>
    </div>
  );
}
...
function Stat({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="rounded-lg border border-border bg-background/40 px-3 py-2">
      ...
      <div className={cn("mt-0.5 text-sm font-semibold", highlight ? "text-[hsl(160_84%_40%)]" : "text-foreground")}>
        {value}
      </div>
    </div>
  );
