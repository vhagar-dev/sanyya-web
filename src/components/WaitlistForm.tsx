    ...
    return (
      <form onSubmit={handleSubmit} className={`flex flex-col sm:flex-row gap-2 ${className}`}>
        <Input
          ...
          required
          disabled={isLoading}
          className="h-11 px-4 bg-secondary/80 backdrop-blur-sm border-border text-foreground placeholder:text-muted-foreground rounded-full focus:bg-secondary focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all flex-1 text-sm disabled:opacity-50"
        />
        <button
          type="submit"
          disabled={isLoading}
          className="h-11 px-6 rounded-full bg-gradient-to-r from-glow-magenta to-glow-pink text-white font-semibold shadow-lg shadow-primary/40 hover:shadow-xl hover:shadow-primary/50 hover:-translate-y-0.5 transition-all shine-effect flex items-center justify-center gap-2 text-sm disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"
        >
          {isLoading ? (
            <>
        ...
        {error && (
          <p className="text-red-400 text-xs mt-1 w-full text-center">{error}</p>
        )}
      </form>
    );
  }
      ...
      <form onSubmit={handleSubmit} className={`flex flex-col gap-3 ${className}`}>
        <div className="flex flex-col sm:flex-row gap-2">
          <Input
            ...
            required
            disabled={isLoading}
            className="h-12 px-5 bg-white/10 backdrop-blur-sm border-white/20 text-white placeholder:text-white/60 rounded-full focus:bg-white/15 focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all flex-1 text-sm disabled:opacity-50"
          />
          <button
            type="submit"
            disabled={isLoading}
            className="h-12 px-7 rounded-full bg-white text-slate-900 font-semibold shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2 text-sm disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"
          >
            {isLoading ? (
              <>
                <Loader2 className="w-4 h-4 animate-spin" />
